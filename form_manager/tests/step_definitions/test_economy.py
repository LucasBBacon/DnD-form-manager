...
@when(
    parsers.parse(
        'the user attempts to buy an item costing {amount:d} "{currency_name}"'
    )
)
...
def check_inventory_value(session_context, expected_value_str, rules_manager):
    ...
    assert (
        actual_cp == expected_cp
    ), f"Expected value {expected_value_str} ({expected_cp} cp), but got {actual_cp} cp"


@then("the purchase should be successful")
def check_purchase_success(session_context):
    assert (
        session_context.get("purchase_success") is True
...
def check_purse_contains(session_context, amount, currency_name):
    ...
    assert (
        char.purse[key] == amount
    ), f"Expected purse to contain {amount} {key}, but found {char.purse[key]} {key}"
