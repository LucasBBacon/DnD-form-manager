...
@when(
    parsers.parse(
        'the user creates an item "{item_name}" using the "{template_name}" template'
    )
)
...
@then(
    parsers.parse(
        'the item "{item_name}" should have a weight of {expected_weight} lbs'
    )
)
def assert_total_weight(session_context, item_name, expected_weight):
    ...
    ), f"Expected weight {expected_weight} lbs, but actual weight is {actual_weight} lbs"
...
def check_item_category(session_context, item_name, category_name):
    ...
    ), f"Expected item category of {expected_category}, but was {actual_category}"
