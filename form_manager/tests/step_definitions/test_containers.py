...
@given(parsers.parse(('the item "{item_name}" is a container with '
...
@given(parsers.parse(('the user has {item_quantity:d} "{item_name}" '
...
def move_item_to_container(session_context, item_quantity, item_name, container_name):
    ...
    char = session_context['character']
    char.inventory.move_item_to_container(
        item_name, container_name, item_quantity)
