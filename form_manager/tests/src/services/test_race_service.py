...
def mock_rules():
    ...
    rules.races = {"human": {"traits": [{"id": "ability_score_increase"}]}}
    rules.traits = {
        "ability_score_increase": {
            "modifiers": [{"type": "ability_bonus", "target": "all", "value": 1}]
        }
    }
    return rules
...
def test_apply_race_finds_subraces(mock_rules):
    mock_rules.races = {
        "elf": {"subraces": {"high_elf": {"traits": [{"id": "cantrip"}]}}}
    }
    mock_rules.traits = {
        "cantrip": {
            "modifiers": [{"type": "spell_choice", "list": "wizard", "level": 0}]
        }
    }
    mock_rules.spells = {"wizard": [["fire_bolt"]]}
    service = RaceService(mock_rules)
    char = Character()
...
def test_ability_bonus_choice_filters_fixed_stats(mock_rules):
    mock_rules.races = {"half_elf": {"traits": [{"id": "stat_boost"}]}}
    mock_rules.traits = {
        "stat_boost": {
            "modifiers": [
                {"type": "ability_bonus", "target": "charisma", "value": 2},
                {"type": "ability_bonus_choice", "count": 2, "amount": 1},
            ]
        }
    }
    service = RaceService(mock_rules)
