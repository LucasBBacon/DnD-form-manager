...
class RaceService:
    ...
    def __apply_traits(self, character: Character, race_node: Dict) -> None:
        for trait_entry in race_node.get("traits", []):
            ...
            label = (
                trait_entry.get("overrides", {}).get("label")
    ...
    def __resolve_choices(
        ...
        mod,
        m_type,
        fixed_bonuses: Optional[List[str]] = None,
    ) -> None:
        choice = None
        match m_type:
            case "language_choice":
                language_options = [
                    lang.get("label", "").lower()
                    for lang in self.rules.languages.values()
                ]
                choice_options = mod.get("pool", language_options)
                choice_options = (
                    language_options if choice_options == "any" else choice_options
                )
            ...
            case "spell_choice":
                ...
                choice = PendingChoice(
                    label=f"{class_key.capitalize()} Spell",
